...
const participantsHtml = details.participants.length > 0
  ? `<ul class="participants-list">${details.participants.map(p => `<li>${p} <button class='delete-btn' data-email='${p}'>🗑️</button></li>`).join('')}</ul>`
...
activityCard.querySelectorAll('.delete-btn').forEach(btn => {
  btn.addEventListener('click', async () => {
    ...
    const activityName = name; // From the loop
    try {
